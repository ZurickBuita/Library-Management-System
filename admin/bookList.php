<?php
  ...
  if(isset($_POST['add_a_book'])) {
    ...
    $book_category = test_input($_POST['book_category']);
    $book_published = test_input(date('Y-m-d', strtotime($_POST['book_published'])));
    $book_copies = test_input($_POST['book_copies']);

    $query = "INSERT INTO books (book_title, book_author, book_category, book_published, book_copies)
     VALUES('$book_title', '$book_author', '$book_category', '$book_published', '$book_copies') ";
    ...
    $ISBN = test_input($_POST['delete_a_book']);
    ...
    $update_ISBN = test_input($_POST['update_ISBN']);
    $book_title = test_input($_POST['book_title']);
    $book_author = test_input($_POST['book_author']);
    $book_category = test_input($_POST['book_category']);
    $book_published = test_input(date('Y-m-d', strtotime($_POST['book_published'])));
                    ...
                    <div class="border border-secondary border-opacity-25 p-3 overflow-auto rounded ">
                        ...
                        <table class="table table-striped table-hover">
                            ...
                            <tbody id="myTable">
                                ...
                                <?php
                                   $query = "SELECT * FROM books WHERE book_is_request = '0' ";
                                   ...
                                   if (mysqli_num_rows($query_run) > 0) {
                                      foreach($query_run as $book) {
                                        ...
                                        <tr>
                                            ...
                                            <td class="d-flex align-items-center m-0 py-0">
                                                <button class="editbtn btn btn-success py-1">Edit</button>&nbsp;&nbsp;
                                                <form class="d-flex align-items-center" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>"
                                                    method="POST">
                                                    <button class="btn btn-danger mt-3 py-1" type="submit" name="delete_a_book"
    ...
    <script>
        $(document).ready(function () {
            $('.editbtn').on('click', function () {
                ...
                $('#update_ISBN').val($.trim(data[0]));
                $('#book_title').val($.trim(data[1]));
                $('#book_author').val($.trim(data[2]));
                $('#book_category').val($.trim(data[3]));
                $('#book_published').val($.trim(data[4]));
                $('#book_copies').val($.trim(data[5]));
            });
